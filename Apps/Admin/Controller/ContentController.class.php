...
use Admin\Model\LearnResourcesModel;
...
class ContentController extends BaseController
{
    ...
    public function index()
    {
        ...
        //获取学习资源栏目指定key,
        //开始处理所有栏目id数组，将学习资源剔除掉
        $all_class_id_list = $classModel->getAllSubId($class_id);
        $key = array_search(45,$all_class_id_list);
        array_splice($all_class_id_list,$key,1);
    ...
    public function editor_resources()
    {
        $Content = D('Content');
        $Con_article = D('Con_article');
        $Class = D("Class");
        if (IS_POST) {
            ...
            if(isset($content_id)){
                ...
                if ($Content->save($_POST)) {
                    ...
                    $this->success('修改成功!', __APP__ . '/Admin/Content/resources');
                } else {
                    $this->error('修改失败!', __APP__ . '/Admin/Content/resources');
                }
            }
        } else {
            if (isset($_GET['content_id'])) {
    ...
    public function resources()
    {
        ...
        //直接将所有栏目ID数组写成学习资源栏目ID，
        $all_class_id_list = array(45);
        /*$key = array_search(45,$all_class_id_list);
        array_splice($all_class_id_list,$key,1);*/
